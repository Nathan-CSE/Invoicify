...
"""
item_format = '<cac:InvoiceLine><cbc:ID>{item_id}</cbc:ID><cbc:Note></cbc:Note><cbc:InvoicedQuantity unitCode="{unit_code}">{amount_product}</cbc:InvoicedQuantity><cbc:LineExtensionAmount currencyID="AUD">{cost_product}</cbc:LineExtensionAmount><cac:Item><cbc:Description>{item_description}</cbc:Description><cbc:Name>{item_name}</cbc:Name><cac:ClassifiedTaxCategory><cbc:ID>GST</cbc:ID><cbc:Percent>{tax_amount}</cbc:Percent><cac:TaxScheme><cbc:ID>{tax_name}</cbc:ID></cac:TaxScheme></cac:ClassifiedTaxCategory></cac:Item><cac:Price><cbc:PriceAmount currencyID="AUD">{cost_per_product}</cbc:PriceAmount></cac:Price></cac:InvoiceLine>'
