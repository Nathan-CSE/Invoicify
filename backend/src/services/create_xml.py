...
def create_xml(file, user):
    ...
    try:
        ...
        print(f"ERROR: {e}")
    invoice = Invoice(name=file["invoiceName"] + ".xml", fields=json.loads(json_val),  rule="AUNZ_PEPPOL_1_0_10", user_id=user.id, is_ready=False, is_gui=True)
