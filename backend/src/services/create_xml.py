...
def format_xml(file: dict) -> dict:
    ...
    for no, item in enumerate(file["invoiceItems"]):
        ...
        products += item_format.format(
            ...
            tax_amount = item["GST"],
            tax_name = "GST",
            after_tax = item["GST"] + item["unitPrice"]
        )
    try:
