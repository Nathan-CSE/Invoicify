...
def test_validate_upload_unsucessful(client, user):
    ...
    assert res.status_code == 400
    assert response_body['message'] == "Validation failed. Failed assertion check: Failed the test cbc:BuyerReference or cac:OrderReference/cbc:ID with error code PEPPOL-EN16931-R003: A buyer reference or purchase order reference MUST be provided. This error happened at /*:Invoice[namespace-uri()='urn:oasis:names:specification:ubl:schema:xsd:Invoice-2'][1]\n\
